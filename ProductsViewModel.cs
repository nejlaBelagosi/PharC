...

{
    ...
    //    {
    //        ProductsList.ItemsSource = Products; // Ako je polje za pretragu prazno, prikaži sve proizvode
    //    }
    //    else
    //    {
    //        var filteredProducts = Products.Where(p => p.Name.Contains(searchText, StringComparison.OrdinalIgnoreCase) || p.Price.ToString().Contains(searchText));
    //       ProductsList.ItemsSource = filteredProducts; // Podesi filtrirane proizvode kao izvor podataka za CollectionView
    //    }
    //}
    public void FilterProducts(string searchText)
    {
